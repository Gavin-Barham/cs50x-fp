...
db.execute(
    "CREATE TABLE IF NOT EXISTS stores (id INT PRIMARY KEY, admin_passcode VARCHAR(255))")
db.execute("INSERT INTO stores (id, admin_passcode) VALUES(?, ?)", 1, '0000')
db.execute("INSERT INTO stores (id, admin_passcode) VALUES(?, ?)", 112, '0000')
db.execute("INSERT INTO stores (id, admin_passcode) VALUES(?, ?)", 48, '0000')

db.execute("CREATE TABLE IF NOT EXISTS users (id BIGINT PRIMARY KEY, store_id INT REFERENCES stores (id), username VARCHAR(255), hash VARCHAR(255))")

db.execute(
    "CREATE TABLE IF NOT EXISTS employees (id BIGINT, store_id INT REFERENCES stores(id), admin BOOLEAN)")
db.execute(
    "CREATE TABLE IF NOT EXISTS orders (store_id INT REFERENCES stores (id), address VARCHAR(40), order_number INTEGER, active Boolean, time TIMESTAMP)")

db.execute(
    "CREATE TABLE IF NOT EXISTS drivers (id INT PRIMARY KEY, store_id INT REFERENCES stores (id), name VARCHAR(30), active Boolean, time TIMESTAMP)")

db.execute("CREATE TABLE IF NOT EXISTS assign (store_id INT REFERENCES stores (id), name VARCHAR(30), address VARCHAR(40), order_number INTEGER, active Boolean, time TIMESTAMP)")
...
def index():
    ...
    active_drivers = db.execute(
        "SELECT name, time FROM drivers WHERE store_id = ? and active = ? ORDER BY time", session["current_store_id"], True)

    # Store a list of all drivers for users store_id for select dropdown
    drivers = db.execute(
        "SELECT name FROM drivers WHERE store_id = ?", session["current_store_id"])

    # Store a list of orders for the associated store_id
    orders = db.execute("SELECT address, order_number FROM orders WHERE store_id = ? AND active = ? ORDER BY order_number",
                        session["current_store_id"], True)
    ...
    if request.method == "POST":
        ...
        if request.form.get("activate") == "add":
            ...
            db.execute("UPDATE drivers SET active = ?, time = ? WHERE name = ? AND store_id = ?",
                       "True", time, request.form.get("driver"), session["current_store_id"])
            return redirect("/")
        ...
        # If user clicked remove driver button
            ...
            db.execute("UPDATE drivers SET active = ? WHERE name = ? AND store_id = ?",
                       "False", request.form.get("driver"), session["current_store_id"])
            return redirect("/")
        ...
        # If user clicked add order button
            ...
            db.execute("INSERT INTO orders (address, order_number, store_id, active, time) VALUES (?, ?, ?, ?, ?)",
                       request.form.get("address"), request.form.get("order_num"), session["current_store_id"], True, time)
            return redirect("/")
        ...
        # If user clicked the remove order button
            ...
            db.execute("UPDATE orders SET active = ? WHERE address = ? AND order_number = ? AND store_id = ?",
                       request.form.get("address"), request.form.get("order_num"), session["current_store_id"])
            return redirect("/")
        ...
        # If user clicked the assign button
            ...
            db.execute("UPDATE assign SET active = ? WHERE store_id = ?",
                       "False", session["current_store_id"])
            ...
            while active_drivers != None or orders != None:
                ...
                if results == None:
                    ...
                    if len(orders) == 1 and len(active_drivers) >= 1:
                        db.execute("INSERT INTO assign (name, address, order_number, store_id, time, active) VALUES (?, ?, ?, ?, ? ,?)",
                                   active_drivers[0]["name"], orders[0]["address"], orders[0]["order_number"], session["current_store_id"], time, True)

                        # Set order in orders to inactive
                        db.execute("UPDATE orders SET active = ? WHERE order_number = ? AND store_id = ?",
                                   "False", orders[0]["order_number"], session["current_store_id"])

                        # Set driver in drivers to inactive
                        db.execute("UPDATE drivers SET active = ? WHERE name = ? AND store_id = ?",
                                   "False", active_drivers[0]["name"], session["current_store_id"])

                        final_results = db.execute(
                            "SELECT * FROM assign WHERE store_id = ? AND active = ?", session["current_store_id"], True)
                ...
                for j in range(len(results)):
                    ...
                    for i in range(1, len(orders)):
                        ...
                        if orders[i]["address"] == results[j]["to"]:
                            ...
                            db.execute("INSERT INTO assign (name, address, order_number, store_id, time, active) VALUES (?, ?, ?, ?, ?, ?)",
                                       active_drivers[0]["name"], orders[i]["address"], orders[i]["order_number"], session["current_store_id"], time, True)

                            # Set order in orders to inactive
                            db.execute("UPDATE orders SET active = ? WHERE order_number = ? AND store_id = ?",
                                       "False", orders[i]["order_number"], session["current_store_id"])
                ...
                # Store a list of orders for the associated store_id
                orders = db.execute(
                    "SELECT address, order_number FROM orders WHERE store_id = ? AND active = ? ORDER BY order_number",  session["current_store_id"], True)
...
def stores():
    ...
    stores = db.execute(
        "SELECT DISTINCT st.id FROM stores st JOIN employees emp ON st.id = emp.store_id WHERE emp.id = ?", session["user_id"])
...
def select_your_stores():
    ...
    stores = db.execute("SELECT DISTINCT id FROM stores")
    if not request.method == "POST":
    ...
    db.execute("INSERT INTO employees (id, store_id) VALUES (?, ?)",
               session["user_id"], session["current_store_id"])
...
def admin():
    ...
    password = db.execute(
        "SELECT admin_passcode FROM stores WHERE id = ?", session["current_store_id"])
    ...
    if request.method == "POST":
        ...
        if request.form.get("store_passcode") == password[0]["admin_passcode"]:
            ...
            db.execute("UPDATE employees SET admin = ? WHERE id = ? AND store_id = ?",
                       True, session["user_id"], session["current_store_id"])
            return redirect("/stores")
...
def drivers():
    ...
    drivers = db.execute(
        "SELECT id, name, store_id FROM drivers WHERE store_id = ?", session["current_store_id"])
    admin = db.execute("SELECT admin From employees WHERE store_id = ? AND id = ?",
                       session["current_store_id"], session["user_id"])
    print(admin[0]["admin"])
    if not request.method == "POST":
    ...
    if request.method == "POST":
        ...
        if admin[0]["admin"] != True:
            ...
            if request.form["app_driver"] == "add":
                ...
                print(driver_id, driver_name)
                # Insert new row into drivers table with applicable data
                db.execute("INSERT INTO drivers (id, name, store_id) VALUES (?, ?, ?)", request.form.get(
                    "id"), request.form.get("name"), session.get("current_store_id"))
            ...
            if request.form["app_driver"] == "remove":
                ...
                db.execute("DELETE FROM drivers WHERE id = ? AND name = ?",
...
def history():
    ...
    history = db.execute(
        "SELECT * FROM assign WHERE store_id = ?", session["current_store_id"])
    return render_template("history.html", history=history)


if __name__ == "__main__":
    app.run(debug=True, port=ENVIRON.PORT, host="0.0.0.0")
